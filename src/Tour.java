...
public class Tour {
	...
	public boolean addNextStopSlicePlusFarDepot(ArrayList<Location> locations) {
		...
		if (!slice1.isEmpty()) {
			...
			if (!slice2.isEmpty()) {
				return locations.isEmpty() ? false
						: addStopSliceDepot(Main.findFarthestLocation(tourStops.get(0), slice2), slice2, locations);
			} else {
				if (!slice3.isEmpty()) {
					return locations.isEmpty() ? false
							: addStopSliceDepot(Main.findFarthestLocation(tourStops.get(0), slice3), slice3, locations);
				} else {
					if (!slice4.isEmpty()) {
						return locations.isEmpty() ? false
								: addStopSliceDepot(Main.findFarthestLocation(tourStops.get(0), slice4), slice4, locations);
					} else {
						if (!slice5.isEmpty()) {
							return locations.isEmpty() ? false
